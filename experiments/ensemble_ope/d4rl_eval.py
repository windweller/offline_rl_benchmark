...
def run_experiment(policy_perfs, fqe_bootstraps, ope_scores):
    ...
    assert "halfcheetah-medium-v2" in fqe_bootstraps
    ...
    for env_name, policy_name_to_est in fqe_bootstraps.items():
        ...
        for policy_name, policy_ope_bootstrap in policy_name_to_est.items():
            ...
            row = [policy_name, env_name] + [policy_perfs[env_name][policy_name]] + list(
                ope_scores_env.flatten() * 1000) + \
                  [score_mat[env_name][policy_name] * 1000] + [switch_ope_mat[env_name][policy_name] * 1000] + [
                      avg_ope_mat[env_name][policy_name] * 1000]
            rows.append(row)
...
def filter_bad_fqes(fqe_bootstraps, filter_for_env='walker2d-medium-v2'):
...
def run_walker2d_experiment(policy_perfs, fqe_bootstraps, ope_scores):
    # collect data to save
    alpha_mat = {}
    score_mat = {}
    switch_ope_mat = {}
...
def run_walker2d_search(n=4):
    ...
    bad_estimators = [1, 2, 3, 4, 12, 13, 14]

    for combo in tqdm(combinations_n):
        if any([i in combo for i in bad_estimators]):
            continue
        ...
        try:
            opera_mse, switch_mse, avg_mse = run_walker2d_experiment(perfs, selected_fqe_boostraps, selected_scores)
            combo_to_mse[combo] = (opera_mse, switch_mse, avg_mse)
        except:
            print("failed")

    pickle.dump(combo_to_mse, open(f"d4rl_results/combo_walker_{n}_to_mse.pkl", "wb"))

def fqe_to_run_walker2d():
    # combo = (5, 6, 7, 8)
    combo = (6, 9, 10, 11)  # early and late, then two early checkpoints
    fqe_bootstraps = load_fqe_bootstrap()
    scores = load_ope_scores()
    perfs = load_policy_perf()
