...
def create_mse_matrix(evaluation_policy, dataset='pomdp-200',
    ...
    if sampling_procedure == 'bootstrap':
        datasets, sepsis, traj_dist, scale_ratio = get_sepsis_boostrap_copies(dataset, n_copies, k_prop=0.1)
    ...
    error_matrix_A = scale_ratio * (1 / n_copies) * np.matmul(pre_A, pre_A.T)
...
def run_exp1(dataset_name='pomdp-200', n_copies=20, ris=False,
    ...
    for i in tqdm(range(len(policies))):
        ...
        ope_scores, ope_bootstrapped_scores, est_bias, est_variance, ope_mse, error_matrix_A = create_mse_matrix(policies[i], ope_scorers=[importance_sampling_scorer, wis_scorer],
                                                   dataset=dataset_name, n_copies=n_copies, ris=ris, efficient_bootstrap=efficient_bootstrap, no_clip=no_clip)

        np.savez(f"sepsis_results/policy_{i}_{dataset_name}_{n_copies}{use_ris}{use_efficient}_ope_bootstrapped_scores.npz", ope_bootstrapped_scores=ope_bootstrapped_scores)
    ...
    opes_mat.to_csv(f'sepsis_results/{dataset_name}_{n_copies}{use_ris}{use_efficient}_opes_mat.csv')
    alpha_mat.to_csv(f'sepsis_results/{dataset_name}_{n_copies}{use_ris}{use_efficient}_alpha_mat.csv')
    sampled_ope_biases_mat.to_csv(f'sepsis_results/{dataset_name}_{n_copies}{use_ris}{use_efficient}_sampled_ope_biases_mat.csv')
    sampled_ope_variances_mat.to_csv(f'sepsis_results/{dataset_name}_{n_copies}_sampled_ope_variances_mat.csv')
    # real_ope_biases_mat.to_csv(f'sepsis_results/{dataset_name}_{n_copies}{use_ris}{use_efficient}_real_ope_biases_mat.csv')
    mse_mat.to_csv(f'sepsis_results/{dataset_name}_{n_copies}{use_ris}{use_efficient}_mse_mat.csv')
    mse_est.to_csv(f'sepsis_results/{dataset_name}_{n_copies}_mse_est.csv')


if __name__ == '__main__':
    ...
    # need to call this to download the policies
    run_exp1('pomdp-200', n_copies=20, efficient_bootstrap=False, no_clip=True) # , efficient_bootstrap=True
    # run_exp1('pomdp-1000', n_copies=20, efficient_bootstrap=False, no_clip=True) # , efficient_bootstrap=True
