...
def create_mse_matrix(evaluation_policy, dataset='pomdp-200',
    ...
    if sampling_procedure == 'bootstrap':
        datasets, sepsis, traj_dist = get_sepsis_boostrap_copies(dataset, n_copies)
    ...
    ope_bootstrapped_scores = np.zeros((mat_n, len(datasets)))
...
def solve_for_alpha(ope_biases: np.array, ope_mses: np.array, ope_scores: np.array, estimated_mse=True):
...
def run_exp2_subsample(dataset_name, n_copies=20):
    # include RIS
    # this is only for the purpose of downloading policies
    datasets = get_sepsis_ensemble_datasets('pomdp-200')
    ...
    for i in tqdm(range(len(policies))):
        ...
        ope_biases, ope_mses, ope_scores, ope_bootstrapped_scores = create_mse_matrix(policies[i], ope_scorers=[importance_sampling_scorer, wis_scorer],
                                                                            dataset=dataset_name, n_copies=n_copies, sampling_procedure='subsample')

        np.savez(f"results/policy_{i}_{dataset_name}_{n_copies}_ope_subsampled_scores.npz", ope_bootstrapped_scores=ope_bootstrapped_scores)
    ...
    opes_mat.to_csv(f'results/{dataset_name}_{n_copies}_subsampled_opes_mat.csv')
    alpha_mat.to_csv(f'results/{dataset_name}_{n_copies}_subsampled_alpha_mat.csv')
    sampled_ope_biases_mat.to_csv(f'results/{dataset_name}_{n_copies}_subsampled_sampled_ope_biases_mat.csv')
    real_ope_biases_mat.to_csv(f'results/{dataset_name}_{n_copies}_subsampled_real_ope_biases_mat.csv')
